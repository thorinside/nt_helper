...
class BusTidyOptimizer {
  ...
  void _applyReplaceModeForBusReuse(List<Connection> optimized, Map<String, BusChange> changes) {
    ...
    for (final entry in busUsage.entries) {
      ...
      if (connections.length == 1) {
        ...
        for (final otherConn in optimized) {
          if (otherConn.id != connection.id &&
              otherConn.sourceAlgorithmIndex == connection.targetAlgorithmIndex &&
      ...
      // Multiple connections on the same bus - be very conservative
        ...
        connections.sort((a, b) {
          ...
          final sourceCompare = a.sourceAlgorithmIndex.compareTo(b.sourceAlgorithmIndex);
        ...
        for (final conn in connections) {
          if (conn.id != lastConnection.id) {
            ...
            if (conn.sourceAlgorithmIndex >= lastConnection.targetAlgorithmIndex) {
              isLastUser = false;
              break;
            }
          }
      ...
      for (final sourceBus in auxBuses) {
        ...
        for (final conn in sourceConnections) {
          if (conn.sourceAlgorithmIndex >= 0) {
            sourceMinSlot = sourceMinSlot == null ? conn.sourceAlgorithmIndex : 
                          (conn.sourceAlgorithmIndex < sourceMinSlot ? conn.sourceAlgorithmIndex : sourceMinSlot);
          }
          if (conn.targetAlgorithmIndex >= 0) {
            sourceMaxSlot = sourceMaxSlot == null ? conn.targetAlgorithmIndex :
                          (conn.targetAlgorithmIndex > sourceMaxSlot ? conn.targetAlgorithmIndex : sourceMaxSlot);
          }
        }
        ...
        for (final conn in targetConnections) {
          if (conn.sourceAlgorithmIndex >= 0) {
            targetMinSlot = targetMinSlot == null ? conn.sourceAlgorithmIndex :
                          (conn.sourceAlgorithmIndex < targetMinSlot ? conn.sourceAlgorithmIndex : targetMinSlot);
          }
          if (conn.targetAlgorithmIndex >= 0) {
            targetMaxSlot = targetMaxSlot == null ? conn.targetAlgorithmIndex :
                          (conn.targetAlgorithmIndex > targetMaxSlot ? conn.targetAlgorithmIndex : targetMaxSlot);
          }
        }
        ...
        for (final conn in sourceConnections) {
          if (conn.sourceAlgorithmIndex >= 0) sourceAlgorithms.add(conn.sourceAlgorithmIndex);
          if (conn.targetAlgorithmIndex >= 0) sourceAlgorithms.add(conn.targetAlgorithmIndex);
        }
        
        for (final conn in targetConnections) {
          if (conn.sourceAlgorithmIndex >= 0) targetAlgorithms.add(conn.sourceAlgorithmIndex);
          if (conn.targetAlgorithmIndex >= 0) targetAlgorithms.add(conn.targetAlgorithmIndex);
        }
