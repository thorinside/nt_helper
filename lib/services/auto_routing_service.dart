...
class AutoRoutingService {
  ...
  Future<BusAssignment> assignBusForConnection({
    ...
    if (sourceAlgorithmIndex == -2) {
      final inputNumber =
          int.tryParse(sourcePortId.replaceAll('physical_input_', '')) ?? 1;
      assignedBus = inputNumber; // Buses 1-12
      debugPrint(
        '[AutoRoutingService] Physical input I$inputNumber connection, using fixed bus $assignedBus',
      );
    }
    // Handle physical output as target (O1-O8 → buses 13-20)
    else if (targetAlgorithmIndex == -3) {
      final outputNumber =
          int.tryParse(targetPortId.replaceAll('physical_output_', '')) ?? 1;
      assignedBus = 12 + outputNumber; // Buses 13-20
      debugPrint(
        '[AutoRoutingService] Physical output O$outputNumber connection, using fixed bus $assignedBus',
      );
    }
    // Handle algorithm output to physical input (bidirectional)
    else if (targetAlgorithmIndex == -2) {
      final inputNumber =
          int.tryParse(targetPortId.replaceAll('physical_input_', '')) ?? 1;
      assignedBus = inputNumber; // Buses 1-12
      debugPrint(
        '[AutoRoutingService] Algorithm to physical input I$inputNumber connection, using fixed bus $assignedBus',
      );
    }
    // Handle physical output as source (bidirectional)
    else if (sourceAlgorithmIndex == -3) {
      final outputNumber =
          int.tryParse(sourcePortId.replaceAll('physical_output_', '')) ?? 1;
      assignedBus = 12 + outputNumber; // Buses 13-20
      debugPrint(
        '[AutoRoutingService] Physical output O$outputNumber as source connection, using fixed bus $assignedBus',
      );
    }
  ...
  int _findAvailableAuxBusConsideringState(
  ...
  int? _getParameterValue(int algorithmIndex, int parameterNumber) {
    ...
    if (val.value > 0) {
      debugPrint(
        '[AutoRoutingService] Parameter #$parameterNumber in algorithm $algorithmIndex has value ${val.value}',
      );
      return val.value;
    }
  ...
  Set<int> _collectUsedOutputBusesFromState() {
    ...
    for (final slot in s.slots) {
      for (final param in slot.parameters) {
        ...
        final isBusParam =
            unit == 'bus' || (param.unit == 1 && param.max >= 28);
        ...
        if (val.value > 0) {
          used.add(val.value);
          debugPrint(
            '[AutoRoutingService] Bus ${val.value} is in use by parameter "${param.name}"',
          );
        }
      }
    }
    ...
    debugPrint(
      '[AutoRoutingService] Total buses in use: ${used.length} - ${used.toList()..sort()}',
    );
    return used;
  }
  ...
  Future<void> updateBusParameters(List<ParameterUpdate> updates) async {
    try {
      // Use parallel updates where possible for better performance
      // The DistingCubit will handle optimistic updates and sync with hardware
      final futures = updates.map((update) {
        debugPrint(
          '[AutoRoutingService] Updating param #${update.parameterNumber} on slot ${update.algorithmIndex} to value ${update.value}',
        );
        return _cubit.updateParameterValue(
  ...
  int _findParameterNumberForPort(
    int algorithmIndex,
    String portId, {
    ...
    if (algorithmIndex == -2 || algorithmIndex == -3) {
      debugPrint(
        '[AutoRoutingService] Physical node $algorithmIndex detected, no parameters to lookup',
      );
    ...
    if (distingState is! DistingStateSynchronized) {
      debugPrint(
        '[AutoRoutingService] Not synchronized, using fallback parameter number',
      );
    ...
    if (algorithmIndex >= distingState.slots.length) {
      debugPrint(
        '[AutoRoutingService] Algorithm index $algorithmIndex out of bounds',
      );
      return isOutput ? 0 : 1; // Fallback
    }
      ...
      if (isOutput) {
        ...
        if (isOutputParam) {
          ...
          if (portId.contains('output') &&
              sanitizedParamName.contains('output')) {
            debugPrint(
              '[AutoRoutingService] Partial match: Found output parameter "${param.name}" (#${param.parameterNumber}) for port "$portId"',
            );
            return param.parameterNumber;
          }
          if (portId.contains('main') && sanitizedParamName.contains('main')) {
            debugPrint(
              '[AutoRoutingService] Partial match: Found main output parameter "${param.name}" (#${param.parameterNumber}) for port "$portId"',
            );
            return param.parameterNumber;
          }
          ...
          final outputParams = slot.parameters.where((p) {
            ...
            return (pUnit == 'bus' || (p.unit == 1 && p.max >= 28)) &&
                (pNameLower.contains('output') ||
                    pNameLower.contains('send') ||
                    pNameLower.contains('main out') ||
                    pNameLower.contains('aux'));
          }).toList();

          if (outputParams.length == 1) {
            debugPrint(
              '[AutoRoutingService] Single output: Found parameter "${param.name}" (#${param.parameterNumber}) for port "$portId"',
            );
            return param.parameterNumber;
          }
        }
        ...
        if (isInputParam) {
          // Check for partial match
          if (portId.contains('input') &&
              sanitizedParamName.contains('input')) {
            // Try to match more specific inputs first
            if (portId.startsWith('l_') &&
                sanitizedParamName.startsWith('l_')) {
              debugPrint(
                '[AutoRoutingService] Partial match: Found L input parameter "${param.name}" (#${param.parameterNumber}) for port "$portId"',
              );
              return param.parameterNumber;
            }
            if (portId.startsWith('r_') &&
                sanitizedParamName.startsWith('r_')) {
              debugPrint(
                '[AutoRoutingService] Partial match: Found R input parameter "${param.name}" (#${param.parameterNumber}) for port "$portId"',
              );
              return param.parameterNumber;
            }
            // Generic input match
            debugPrint(
              '[AutoRoutingService] Partial match: Found input parameter "${param.name}" (#${param.parameterNumber}) for port "$portId"',
            );
            return param.parameterNumber;
          }
          // Check for specific parameter names
          if (portId.contains('pitch') &&
              sanitizedParamName.contains('pitch')) {
            debugPrint(
              '[AutoRoutingService] Partial match: Found pitch parameter "${param.name}" (#${param.parameterNumber}) for port "$portId"',
            );
            return param.parameterNumber;
          }
          if (portId.contains('wave') && sanitizedParamName.contains('wave')) {
            debugPrint(
              '[AutoRoutingService] Partial match: Found wave parameter "${param.name}" (#${param.parameterNumber}) for port "$portId"',
            );
            return param.parameterNumber;
          }
          ...
          final inputParams = slot.parameters.where((p) {
            ...
            final pNameLower = p.name.toLowerCase();
            return (pUnit == 'bus' || (p.unit == 1 && p.max >= 28)) &&
                (pNameLower.contains('input') ||
                    pNameLower.contains('receive') ||
                    pNameLower.contains('pitch') ||
                    pNameLower.contains('wave'));
          }).toList();

          if (inputParams.length == 1) {
            debugPrint(
              '[AutoRoutingService] Single input: Found parameter "${param.name}" (#${param.parameterNumber}) for port "$portId"',
            );
            return param.parameterNumber;
          }
        }
...
extension AutoRoutingServiceExtensions on AutoRoutingService {
  ...
  Future<void> removeConnection({
    ...
    debugPrint(
      '[AutoRoutingService] Removing connection from $sourceAlgorithmIndex:$sourcePortId to $targetAlgorithmIndex:$targetPortId',
    );

    // Clear algorithm parameters individually, skip physical nodes (negative indices)
    // This allows clearing algorithm parameters when disconnecting from physical I/O
    // while avoiding RangeError on physical nodes

    // Clear target input parameter if target is an algorithm (not physical node)
    if (targetAlgorithmIndex >= 0) {
      final targetParamNumber = _findParameterNumberForPort(
        targetAlgorithmIndex,
        targetPortId,
      ...
      debugPrint(
        '[AutoRoutingService] Cleared input parameter #$targetParamNumber on algorithm $targetAlgorithmIndex',
      );
    } else {
      debugPrint(
        '[AutoRoutingService] Target is physical node $targetAlgorithmIndex, skipping parameter update',
      );
    }

    // Clear source output parameter if source is an algorithm (not physical node)
    if (sourceAlgorithmIndex >= 0) {
      final sourceParamNumber = _findParameterNumberForPort(
        sourceAlgorithmIndex,
        sourcePortId,
      ...
      debugPrint(
        '[AutoRoutingService] Cleared output parameter #$sourceParamNumber on algorithm $sourceAlgorithmIndex',
      );
    } else {
      debugPrint(
        '[AutoRoutingService] Source is physical node $sourceAlgorithmIndex, skipping parameter update',
