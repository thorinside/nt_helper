...
class BusDependencyGraph {
  ...
  List<SafeReplaceOpportunity> findSafeReplaceOpportunities() {
    ...
    for (final lifetime in _busLifetimes.values) {
      if (lifetime.writerSlots.isEmpty || lifetime.readerSlots.isEmpty) continue;
  ...
  SafeReplaceOpportunity? _findOptimalReplaceConnection(PreciseBusLifetime lifetime) {
    ...
    final candidateWriters = lifetime.writerSlots
        .where((writerSlot) => writerSlot <= lastReader)
        .toList()
      ..sort((a, b) => b.compareTo(a)); // Sort in descending order (latest first)
    ...
    for (final writerSlot in candidateWriters) {
      ...
      if (_canSafelyApplyReplace(bus, writerSlot, lastReader)) {
        ...
        final connection = lifetime.connections.firstWhere(
          (conn) => conn.sourceAlgorithmIndex == writerSlot && conn.assignedBus == bus,
  ...
  List<int> getBusesAvailableForReuse(List<SafeReplaceOpportunity> replaceOpportunities) {
  ...
  bool validateSignalIntegrity(List<Connection> originalConnections, List<Connection> optimizedConnections) {
    ...
    for (final originalPath in originalPaths) {
      final matchingPaths = optimizedPaths.where((optimizedPath) => _pathsEquivalent(originalPath, optimizedPath)).toList();
    ...
    for (final entry in busUsage.entries) {
      if (entry.value.length > 1 && !_areBusSharingConnectionsSafe(entry.value)) {
...
class PreciseBusLifetime {
  ...
  @override
  String toString() => 'BusLifetime(bus: $bus, writers: $writerSlots, readers: $readerSlots)';
}
...
class SafeReplaceOpportunity {
  ...
  @override
  String toString() => 'SafeReplaceOpportunity(bus: $bus, freeAfter: $freeAfterSlot, freeable: $busesFreeable)';
}
