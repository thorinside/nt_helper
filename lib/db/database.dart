...
class AppDatabase extends _$AppDatabase {
  ...
  @override
  int get schemaVersion => 14; // Adds compact repeat-aware algorithm metadata
  ...
  MigrationStrategy get migration => MigrationStrategy(
    ...
    onUpgrade: (Migrator m, int from, int to) async {
      ...
      // Migration for version 13: persist the specification values used to
      // instantiate each preset slot. Existing slots intentionally have no
      // rows because their original values cannot be reconstructed safely.
      if (from <= 12) {
        await m.createTable(presetSpecificationValues);
      }

      // Migration for version 14: compact specification-aware repeat grammar.
      // Existing flat metadata remains valid and intentionally gets no grammar.
      if (from <= 13) {
        await m.createTable(algorithmRepeatGrammars);
      }
    },
  );
