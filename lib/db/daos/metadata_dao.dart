...
class MetadataDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<void> upsertAlgorithms(List<AlgorithmEntry> entries) {
    return batch((batch) {
      batch.insertAllOnConflictUpdate(algorithms, entries);
    });
  }
  ...
  Future<AlgorithmRepeatGrammarEntry?> getAlgorithmRepeatGrammar(String guid) {
    return (select(
      algorithmRepeatGrammars,
    )..where((row) => row.algorithmGuid.equals(guid))).getSingleOrNull();
  }

  Future<List<AlgorithmRepeatGrammarEntry>> getAllAlgorithmRepeatGrammars() {
    return (select(
      algorithmRepeatGrammars,
    )..orderBy([(row) => OrderingTerm.asc(row.algorithmGuid)])).get();
  }

  /// Atomically replaces one algorithm's flat shape and matching repeat grammar.
  Future<void> replaceAlgorithmShapeAndGrammar({
    ...
    String? encodedGrammar;
    if (grammar != null) {
      final compact = grammar.toCompactJson();
      AlgorithmRepeatGrammar.fromCompactJson(compact);
      encodedGrammar = jsonEncode(compact);
    }

    await transaction(() async {
      await (delete(
        algorithmRepeatGrammars,
      )..where((row) => row.algorithmGuid.equals(guid))).go();
      await (delete(
        parameterOutputModeUsage,
      )..where((row) => row.algorithmGuid.equals(guid))).go();
      await (delete(
      ...
      await batch((batch) {
        batch.insertAll(this.parameters, parameters);
        batch.insertAll(parameterEnums, enums);
        batch.insertAll(parameterPages, pages);
        batch.insertAll(parameterPageItems, pageItems);
        batch.insertAll(parameterOutputModeUsage, outputUsage);
      });
      if (grammar != null) {
        await into(algorithmRepeatGrammars).insert(
          AlgorithmRepeatGrammarEntry(
            algorithmGuid: guid,
            grammarVersion: AlgorithmRepeatGrammar.currentVersion,
            grammarJson: encodedGrammar!,
          ),
        );
      }
    });
  ...
  Future<void> clearAllMetadata() async {
    ...
    await delete(algorithmRepeatGrammars).go();
    await delete(parameterOutputModeUsage).go();
