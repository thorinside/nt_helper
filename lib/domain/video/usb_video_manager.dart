...
class UsbVideoManager {
  ...
  Future<void> connectToDevice(String deviceId) async {
    try {
      ...
      debugPrint('[UsbVideoManager] Video stream state updated to streaming - VideoFrameCubit will handle frame consumption');
      
    } catch (e) {
      debugPrint('[UsbVideoManager] Connection failed: $e');
      _updateState(VideoStreamState.error('Device disconnected or unavailable'));
      _startRecoveryTimer();
    }
  }
  ...
  Future<void> autoConnect() async {
    ...
    if (distingNT != null) {
      ...
      for (final device in devices) {
        debugPrint('[UsbVideoManager] Device: ${device.productName} (${device.deviceId})');
      }
      if (devices.isNotEmpty) {
        debugPrint('[UsbVideoManager] Connecting to first device: ${devices.first.deviceId}');
        await connectToDevice(devices.first.deviceId);
      } else {
        _updateState(const VideoStreamState.error('No USB video devices found'));
      }
    }
  }
  ...
  void _startRecoveryTimer() {
    ...
    debugPrint('[UsbVideoManager] Starting recovery timer - checking every ${_recoveryCheckInterval.inSeconds} seconds');
    ...
    _recoveryTimer = Timer.periodic(_recoveryCheckInterval, (timer) async {
      ...
      if (_lastConnectedDeviceId != null) {
        final devices = await listUsbCameras();
        final targetDevice = devices.where((d) => d.deviceId == _lastConnectedDeviceId).firstOrNull;
        
        if (targetDevice != null) {
          debugPrint('[UsbVideoManager] Last connected device found, attempting reconnection');
