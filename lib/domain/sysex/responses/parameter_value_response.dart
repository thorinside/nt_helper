...
class ParameterValueResponse extends SysexResponse {
  ...
  ParameterValue parse() {
    // Extract disabled flag from byte0 of the value encoding
    // Bits 16-20 of the 21-bit parameter value contain the flag
    // Formula: flag = (byte0 >> 2) & 0x1F
    // flag == 1 means parameter is disabled
    final byte0 = data[4];
    final isDisabled = _extractDisabledFlag(byte0);

    return ParameterValue(
      algorithmIndex: decode8(data.sublist(0, 1)),
      parameterNumber: decode16(data, 1),
      value: _extractValue(data, 4),
