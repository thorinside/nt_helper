...
class _RoutingCanvasState extends State<RoutingCanvas> {
  ...
  static const double _canvasExpansion =
      1000.0; // How much to expand when needed
  ...
  bool _finalizingConnection =
      false; // Prevent double create on pointer up + pan end
  ...
  Size _calculateRequiredCanvasBounds() {
    ...
    if (state is NodeRoutingStateLoaded) {
      ...
      final physicalOutputPosition =
          state.physicalOutputPosition ??
          const NodePosition(
            x: 700.0,
            y: 100.0,
            width: 80.0,
      ...
      final outputRight =
          physicalOutputPosition.x +
          physicalOutputPosition.width +
          _canvasExpansion;
      final outputBottom =
          physicalOutputPosition.y +
          physicalOutputPosition.height +
          _canvasExpansion;
  ...
  void _updateCanvasSizeIfNeeded() {
    ...
    if (requiredSize.width > _canvasSize.width ||
        requiredSize.height > _canvasSize.height) {
      final newSize = Size(
      ...
      debugPrint(
        '[RoutingCanvas] Expanding canvas from ${_canvasSize.width.toInt()}x${_canvasSize.height.toInt()} to ${newSize.width.toInt()}x${newSize.height.toInt()}',
      );
        ...
        child: Stack(
          children: [
            ...
            GestureDetector(
              ...
              behavior:
                  HitTestBehavior.opaque, // Catch events in empty space only
            ...
            ...widget.nodePositions.entries.map((entry) {
              ...
              return AlgorithmNodeWidget(
                ...
                onMoveUp: canMoveUp
                    ? () => _handleMoveAlgorithmUp(algorithmIndex)
                    : null,
                onMoveDown: canMoveDown
                    ? () => _handleMoveAlgorithmDown(algorithmIndex)
                    : null,
                  ...
                  child: PhysicalOutputNodeWidget(
                    ...
                    onPositionChanged: (newPosition) {
                      context
                          ...
                          .updatePhysicalOutputPosition(newPosition);
  ...
  void _handlePortConnectionStart(
    ...
    if (type == PortType.output) {
      ...
      cubit.startConnectionPreview(
        ...
        portId,
        Offset
            .zero, // Initial cursor position, will be updated on first pan update
      );
    }
  }
  ...
  void _handlePortPanStart(
    ...
    if (type == PortType.output) {
      ...
      debugPrint(
        '[RoutingCanvas] Starting connection from port $portKey at $portPosition',
      );
  ...
  void _handlePortPanUpdate(
    ...
    if (type == PortType.output) {
      ...
      if (currentState is NodeRoutingStateLoaded &&
        ...
        debugPrint(
          '[RoutingCanvas] Pan update: global=${details.globalPosition}, canvas=$canvasPosition',
        );
  ...
  void _handleCanvasPanStart(DragStartDetails details) {
    ...
    if (nodeAtPosition == null) {
      ...
      debugPrint(
        '[RoutingCanvas] Pan start at global: ${details.globalPosition}, local: ${details.localPosition}',
      );
    }
  }
  ...
  void _handleCanvasPanUpdate(DragUpdateDetails details) {
    ...
    debugPrint(
      '[RoutingCanvas] Pan update - globalDelta: $globalDelta, details.delta: ${details.delta}',
    );
    ...
    if (widget.horizontalScrollController != null &&
      ...
      final newOffset =
          currentOffset - globalDelta.dx; // Invert: drag right = scroll left
    ...
    if (widget.verticalScrollController != null &&
      ...
      final newOffset =
          currentOffset - globalDelta.dy; // Invert: drag down = scroll up
  ...
  void _applyMomentum(Velocity velocity) {
    ...
    const int frames = 20;
    double velocityX =
        -velocity.pixelsPerSecond.dx / 10; // Inverted and scaled down
  ...
  void _handleGlobalPointerUp(PointerUpEvent event) async {
    ...
    if (currentState is NodeRoutingStateLoaded &&
      ...
      if (canvasBox != null) {
        ...
        String? hoveredPort = hoveredAlg != null
            ? cubit.getPortAtPosition(canvasPos, hoveredAlg)
            : null;
        ...
        if (hoveredAlg != null && hoveredPort != null) {
          ...
          if (stateForCheck is NodeRoutingStateLoaded) {
            ...
            final isInput =
                layout?.inputPorts.any(
                  (p) => (p.id ?? p.name) == hoveredPort,
                ) ??
                false;
            if (!isInput) {
              hoveredPort = null;
  ...
  void _handleConnectionHover(PointerEvent event) {
    ...
    if (_connectionPainter != null) {
      hoveredLabelId = _connectionPainter!.getLabelAtPosition(
        event.localPosition,
      );
    }
  ...
  void _handleConnectionTapDown(TapDownDetails details) {
    ...
    if (_connectionPainter != null) {
      clickedLabelId = _connectionPainter!.getLabelAtPosition(
  ...
  Connection? _getConnectionAtPosition(Offset position) {
    ...
    for (final connection in widget.connections) {
      ...
      if (_isPointNearBezier(position, sourcePos, targetPos, tolerance: 15.0)) {
  ...
  void _handleLabelClick(String labelId) {
    ...
    if (labelId.startsWith('connection_') && labelId.endsWith('_mode')) {
      ...
      debugPrint(
        '[RoutingCanvas] Label clicked: $labelId -> connectionId: $connectionId',
      );
