...
sealed class RoutingState with _$RoutingState {
  ...
  factory RoutingState.fromJson(Map<String, dynamic> json) => _$RoutingStateFromJson(json);
  ...
  int get activeConnectionCount => connections.where((conn) => conn.isActive).length;
  
  /// Returns the total number of connections with errors
  int get errorConnectionCount => connections.where((conn) => conn.hasError).length;
  ...
  List<Connection> getInputConnectionsForPort(String portId) {
    return connections.where(
      (conn) => conn.destinationPortId == portId
    ).toList();
  }
  ...
  List<Connection> getOutputConnectionsForPort(String portId) {
    return connections.where(
      (conn) => conn.sourcePortId == portId
    ).toList();
  }
  
  /// Creates a copy with updated status
  RoutingState withStatus(RoutingSystemStatus newStatus, {String? errorMessage}) {
    return copyWith(
      status: newStatus,
  ...
  RoutingState withRemovedConnection(String connectionId) {
    return copyWith(
      connections: connections.where((conn) => conn.id != connectionId).toList(),
      lastUpdated: DateTime.now(),
    );
  }
