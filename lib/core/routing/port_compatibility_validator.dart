...
class ValidationError {
  ...
  @override
  int get hashCode =>
      Object.hash(type, message, sourcePortId, destinationPortId, details);
}
...
class ValidationWarning {
  ...
  @override
  int get hashCode =>
      Object.hash(message, sourcePortId, destinationPortId, details);
}

/// Custom validation rule function type
typedef ValidationRule =
    ValidationResult Function(Port source, Port destination);
...
class PortCompatibilityValidator {
  ...
  PortCompatibilityValidator({List<ValidationRule> customRules = const []})
    ...
    if (!source.canConnectTo(destination)) {
      return ValidationResult.failure([
        ValidationError(
          ...
          message:
              'Cannot connect ${source.direction.name} port to ${destination.direction.name} port',
          sourcePortId: source.id,
          destinationPortId: destination.id,
        ),
      ]);
  ...
  ValidationResult _validateType(Port source, Port destination) {
    ...
    if (source.type != destination.type) {
      warnings.add(
        ValidationWarning(
          message:
              'Cross-type connection: ${source.type.name} to ${destination.type.name}',
          sourcePortId: source.id,
          destinationPortId: destination.id,
        ),
      );
  ...
  ValidationResult _validateConstraints(Port source, Port destination) {
    ...
    if (source.constraints != null) {
      ...
      if (sourceConstraints.containsKey('voltageRange') &&
        ...
        final sourceRange =
            sourceConstraints['voltageRange'] as Map<String, dynamic>?;
        final destRange =
            destination.constraints!['voltageRange'] as Map<String, dynamic>?;
  ...
  ValidationResult _validateExistingConnections(
    ...
    final sourceConnections = existingConnections
        .where(
          (conn) =>
              conn.sourcePortId == source.id ||
              conn.destinationPortId == source.id,
        )
        .length;

    final destConnections = existingConnections
        .where(
          (conn) =>
              conn.sourcePortId == destination.id ||
              conn.destinationPortId == destination.id,
        )
        .length;
    ...
    if (sourceConnections > 0) {
      warnings.add(
        ValidationWarning(
          message:
              'Source port ${source.id} already has $sourceConnections connection(s)',
          sourcePortId: source.id,
          destinationPortId: destination.id,
        ),
      );
    ...
    if (destConnections > 0) {
      warnings.add(
        ValidationWarning(
          message:
              'Destination port ${destination.id} already has $destConnections connection(s)',
          sourcePortId: source.id,
          destinationPortId: destination.id,
        ),
      );
