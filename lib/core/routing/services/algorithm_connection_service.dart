...
class AlgorithmConnectionService {
  ...
  List<AlgorithmConnection> discoverAlgorithmConnections(List<Slot> slots) {
    ...
    debugPrint('AlgorithmConnectionService: Discovering connections for ${slots.length} slots');
    ...
    try {
      ...
      debugPrint('AlgorithmConnectionService: Found ${connections.length} algorithm connections');
      return connections;
    } catch (e) {
      debugPrint('AlgorithmConnectionService: Error discovering connections: $e');
      return [];
    }
  }
  ...
  List<AlgorithmConnection> _performConnectionDiscovery(List<Slot> slots) {
    ...
    for (int sourceIndex = 0; sourceIndex < slotBusInfos.length; sourceIndex++) {
      final sourceBusInfo = slotBusInfos[sourceIndex];
      
      for (int targetIndex = 0; targetIndex < slotBusInfos.length; targetIndex++) {
  ...
  _SlotBusInfo _extractBusInfo(Slot slot, int algorithmIndex) {
    ...
    for (final param in slot.parameters) {
      ...
      final busValue = valueByParam[param.parameterNumber] ?? param.defaultValue;
      ...
      if (_isOutputParameter(paramName)) {
        ...
        debugPrint('AlgorithmConnectionService: Found OUTPUT parameter "${param.name}" -> bus $busValue');
      } else if (_isInputParameter(paramName)) {
        inputBuses[param.name] = busValue;
        debugPrint('AlgorithmConnectionService: Found INPUT parameter "${param.name}" -> bus $busValue');
      }
    }
    ...
    for (final sourceOutput in sourceBusInfo.outputBuses.entries) {
      ...
      for (final targetInput in targetBusInfo.inputBuses.entries) {
        ...
        if (sourceBus == targetBus) {
          final connectionType = _inferConnectionType(sourceOutput.key, targetInput.key);
  ...
  AlgorithmConnectionType _inferConnectionType(String sourceParam, String targetParam) {
    ...
    // Check for gate/trigger connections
    if (sourceLower.contains('gate') || sourceLower.contains('trigger') ||
        targetLower.contains('gate') || targetLower.contains('trigger')) {
      return AlgorithmConnectionType.gateTrigger;
    }
    ...
    if (sourceLower.contains('audio') || sourceLower.contains('main') ||
        sourceLower.contains('left') || sourceLower.contains('right') ||
        targetLower.contains('audio') || targetLower.contains('wave')) {
      return AlgorithmConnectionType.audioSignal;
    }
  ...
  bool _isValidConnection(AlgorithmConnection connection) {
    ...
    if (connection.busNumber < _minBusNumber || connection.busNumber > _maxBusNumber) {
      return false;
    }
  ...
  List<AlgorithmConnection> _sortConnectionsDeterministically(List<AlgorithmConnection> connections) {
