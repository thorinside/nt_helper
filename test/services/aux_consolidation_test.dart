...
void main() {
  ...
  group('Aux Bus Consolidation - Real Scenario', () {
    ...
    test(
      'should consolidate non-overlapping Aux buses from routing analysis',
      () async {
        ...
        for (final change in result.changes.values) {
          debugPrint(
            '  ${change.connectionId}: bus ${change.oldBus} -> ${change.newBus}, replace: ${change.oldReplaceMode} -> ${change.newReplaceMode}',
          );
        }
        ...
        expect(
          optimizedBuses.length,
          lessThan(originalBuses.length),
        ...
        for (final conn in result.optimizedConnections) {
          if (result.changes.containsKey(conn.id) &&
              result.changes[conn.id]!.oldBus !=
                  result.changes[conn.id]!.newBus) {
            expect(
              conn.replaceMode,
              isTrue,
