...
void main() {
  ...
  group('AutoRoutingService - Port ID Suffix Handling', () {
    ...
    test('should handle stereo VCF port IDs with L/R suffixes from UI', () async {
      ...
      expect(result.sourceBus, greaterThanOrEqualTo(21)); // Should use aux bus
    });

    test('should handle UI dragging to specific channel of stereo target', () async {
    ...
    test('should not confuse algorithm input with physical output when port ID has suffix', () async {
      ...
      expect(targetUpdates.first.algorithmIndex, equals(1)); // VCF, not physical
    });
      ...
      when(mockCubit.state).thenReturn(
        DistingState.synchronized(
          ...
          slots: [
            ...
            Slot(
              algorithm: Algorithm(algorithmIndex: 1, guid: 'mult', name: 'Multi-channel'),
              routing: RoutingInfo(algorithmIndex: 1, routingInfo: []),
              pages: ParameterPages(algorithmIndex: 1, pages: []),
              parameters: [
                ParameterInfo(
