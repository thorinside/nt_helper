...
void main() {
  ...
  group('AutoRoutingService - Physical Node Connections', () {
    ...
    group('physical node connection handling', () {
      test(
        'should handle physical input ports I1-I12 without RangeError',
        () async {
      ...
      test(
        'should handle physical output ports O1-O8 without RangeError',
        () async {
    ...
    group('removeConnection with physical nodes', () {
      test(
        'should clear algorithm parameter when removing physical input to algorithm connection',
        () async {
      ...
      test(
        'should clear algorithm parameter when removing algorithm to physical output connection',
        () async {
      ...
      test(
        'should skip all parameters when removing physical input to physical output connection',
        () async {
    ...
    group('createConnection with physical nodes', () {
      test(
        'should update algorithm parameter when creating physical input to algorithm connection',
        () async {
          ...
          expect(
            result.parameterUpdates.first.algorithmIndex,
            equals(0),
          ); // Target algorithm
          expect(
            result.parameterUpdates.first.value,
            equals(4),
          ); // Bus 4 for I4

          // No physical source parameter update (negative index skipped)
          expect(
            result.parameterUpdates.where((p) => p.algorithmIndex == -2),
            isEmpty,
          );
        },
      );
      ...
      test(
        'should update algorithm parameter when creating algorithm to physical output connection',
        () async {
          ...
          expect(
            result.parameterUpdates.where((p) => p.algorithmIndex == -3),
            isEmpty,
          );
        },
      );
      ...
      test(
        'should skip all parameters when creating physical input to physical output connection',
        () async {
    ...
    group('edge cases and boundary conditions', () {
      test(
        'should not throw RangeError with negative indices in non-synchronized state',
        () async {
      ...
      test(
        'should handle mixed physical/algorithm connection removal gracefully',
        () async {
