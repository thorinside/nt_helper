...
void main() {
  ...
  group('AutoRoutingService - Algorithm Index Confusion', () {
    ...
    test(
      'should distinguish between algorithm index and physical node index',
      () async {
    ...
    test(
      'should handle VCF at various slot positions without confusing with physical nodes',
      () async {
        ...
        for (int vcfSlot in [0, 1, 2, 3, 4, 5]) {
          ...
          expect(
            result.sourceBus,
            isNot(inInclusiveRange(13, 20)),
            reason:
                'VCF at slot $vcfSlot incorrectly routed to physical output bus',
          );
          expect(
            result.sourceBus,
    ...
    test(
      'should correctly interpret port IDs that might look like physical outputs',
      () async {
