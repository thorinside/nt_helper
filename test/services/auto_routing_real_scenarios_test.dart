...
void main() {
  ...
  group('AutoRoutingService - Real World Scenarios', () {
    ...
    group('Poly Multisample to Stereo VCF', () {
      test(
        'should correctly connect Poly Multisample outputs to stereo VCF inputs',
        () async {
      ...
      test(
        'should handle explicit stereo output to stereo input connection',
        () async {
          ...
          expect(
            result.assignedBuses[1],
            equals(result.assignedBuses[0] + 1),
          ); // Consecutive
      ...
      test(
        'should not mistakenly route to physical outputs when target is algorithm',
        () async {
          ...
          final result = await service.assignBusForConnection(
            ...
            targetAlgorithmIndex:
                1, // This is an algorithm, not -3 (physical output)
            targetPortId: '0',
            existingConnections: [],
          );
    ...
    group('Port ID Matching Issues', () {
      test(
        'should correctly match port IDs for width-aware algorithms',
        () async {
          ...
          final targetUpdate = result.parameterUpdates.firstWhere(
            (u) => u.algorithmIndex == 1,
          );
    ...
    group('Complex Multi-Algorithm Scenarios', () {
      test('should handle chain of width-aware algorithms', () async {
        ...
        expect(
          result2.assignedBuses[0],
          isNot(equals(result1.assignedBuses[0])),
        );
      });
    });
  });
