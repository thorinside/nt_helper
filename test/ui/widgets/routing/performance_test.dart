...
void main() {
  group('Performance Tests', () {
    ...
    group('Drag Operation Performance', () {
      testWidgets('Algorithm node drag operations perform efficiently', (tester) async {
        final stopwatch = Stopwatch();
        final List<Duration> dragTimes = [];
        
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(
              body: AlgorithmNodeWidget(
                algorithmName: 'Performance Test Algorithm',
                slotNumber: 1,
                position: const Offset(200, 200),
                inputLabels: ['Input 1', 'Input 2', 'Input 3'],
                outputLabels: ['Output 1', 'Output 2'],
                onDragStart: () => stopwatch.start(),
                onDragEnd: () {
                  stopwatch.stop();
                  dragTimes.add(Duration(microseconds: stopwatch.elapsedMicroseconds));
                  stopwatch.reset();
                },
              ),
            ),
        ...
        for (int i = 0; i < 5; i++) {
          await tester.dragFrom(
            ...
            Offset(20.0 * i, 20.0 * i),
          );
          await tester.pumpAndSettle();
        }

        // Drag operations should complete quickly
        expect(dragTimes, hasLength(5));
        final avgDragTime = dragTimes.map((d) => d.inMicroseconds).reduce((a, b) => a + b) / dragTimes.length;
        
        // Each drag should complete within 100ms (100,000 microseconds)
        expect(avgDragTime, lessThan(100000), 
            reason: 'Average drag time should be under 100ms, was ${avgDragTime / 1000}ms');
        
        // No single drag should take more than 200ms
        final maxDragTime = dragTimes.map((d) => d.inMicroseconds).reduce((a, b) => a > b ? a : b);
        expect(maxDragTime, lessThan(200000),
            reason: 'Max drag time should be under 200ms, was ${maxDragTime / 1000}ms');
      });

      testWidgets('Physical I/O node drag operations perform efficiently', (tester) async {
        final List<Duration> physicalDragTimes = [];
        final stopwatch = Stopwatch();
                ...
                children: [
                  Positioned(
                    ...
                    child: PhysicalInputNode(
                      ...
                      onNodeDragStart: () => stopwatch.start(),
                      onNodeDragEnd: () {
                        stopwatch.stop();
                        physicalDragTimes.add(Duration(microseconds: stopwatch.elapsedMicroseconds));
                        stopwatch.reset();
                      },
                    ),
                  ),
        ...
        // Perform multiple physical node drag operations
        for (int i = 0; i < 3; i++) {
          await tester.dragFrom(
            tester.getCenter(find.byType(PhysicalInputNode)),
            Offset(15.0 * i, 15.0 * i),
          );
          await tester.pumpAndSettle();
        }

        // Physical I/O drag operations should be efficient despite having many ports
        expect(physicalDragTimes, hasLength(3));
        final avgPhysicalDragTime = physicalDragTimes
            .map((d) => d.inMicroseconds)
            .reduce((a, b) => a + b) / physicalDragTimes.length;
        
        // Physical nodes have 12 ports, so allow slightly more time
        expect(avgPhysicalDragTime, lessThan(150000),
            reason: 'Average physical drag time should be under 150ms, was ${avgPhysicalDragTime / 1000}ms');
      });

      testWidgets('Simultaneous node drags maintain performance', (tester) async {
        ...
        void recordDragTime(String nodeType) {
          ...
          stopwatch.stop();
          dragTimesByNodeType.putIfAbsent(nodeType, () => [])
                ...
                children: [
                  ...
                  Positioned(
                    ...
                    child: PhysicalInputNode(
                      ...
                      onNodeDragStart: () => stopwatches['physical_input']!.start(),
                  ...
                  Positioned(
                    ...
                    child: PhysicalOutputNode(
                      ...
                      onNodeDragStart: () => stopwatches['physical_output']!.start(),
        ...
        for (final nodeType in dragTimesByNodeType.keys) {
          ...
          expect(times, isNotEmpty, reason: '$nodeType should have recorded drag times');
          
          final avgTime = times.map((d) => d.inMicroseconds).reduce((a, b) => a + b) / times.length;
          expect(avgTime, lessThan(200000),
              reason: '$nodeType drag should be under 200ms even with concurrent operations');
        }
      });
    });

    group('Port Position Update Performance', () {
      testWidgets('Port position callbacks execute efficiently', (tester) async {
          ...
          MaterialApp(
            home: Scaffold(
              body: AlgorithmNodeWidget(
                ...
                inputLabels: List.generate(8, (i) => 'Input ${i + 1}'), // Many inputs
                outputLabels: List.generate(8, (i) => 'Output ${i + 1}'), // Many outputs
                ...
                onPortPositionResolved: (portId, position, isInput) {
                  ...
                  callbackTimes.add(Duration(microseconds: stopwatch.elapsedMicroseconds));
                  stopwatch.reset();
                },
              ),
            ),
        ...
        final avgCallbackTime = callbackTimes.map((d) => d.inMicroseconds).reduce((a, b) => a + b) / callbackTimes.length;
        expect(avgCallbackTime, lessThan(1000), // 1ms per callback
            reason: 'Port position callbacks should execute in under 1ms on average');

        // No callback should take more than 5ms
        final maxCallbackTime = callbackTimes.map((d) => d.inMicroseconds).reduce((a, b) => a > b ? a : b);
        expect(maxCallbackTime, lessThan(5000),
            reason: 'No port callback should take more than 5ms');
      });

      testWidgets('Rapid position updates are handled efficiently', (tester) async {
        int positionUpdateCount = 0;
        final List<Duration> updateIntervals = [];
        DateTime? lastUpdateTime;

        Offset nodePosition = const Offset(100, 100);

        Widget buildRapidUpdateScene() {
          return MaterialApp(
            home: Scaffold(
              body: Positioned(
                ...
                top: nodePosition.dy,
                child: AlgorithmNodeWidget(
                  algorithmName: 'Rapid Update Test',
                  slotNumber: 1,
                  position: nodePosition,
                  inputLabels: ['Rapid Input'],
                  inputPortIds: ['rapid_test'],
                  onPortPositionResolved: (portId, pos, isInput) {
                    final now = DateTime.now();
                    if (lastUpdateTime != null) {
                      updateIntervals.add(now.difference(lastUpdateTime!));
                    }
                    lastUpdateTime = now;
                    positionUpdateCount++;
                  },
        ...
        // Initial render
        await tester.pumpWidget(buildRapidUpdateScene());
        await tester.pumpAndSettle();
        await tester.pump();
        ...
        for (int i = 1; i <= 20; i++) {
          nodePosition = Offset(100.0 + i * 5, 100.0 + i * 3);
          await tester.pumpWidget(buildRapidUpdateScene());
          await tester.pump(const Duration(milliseconds: 5)); // Very rapid updates
        }
        ...
        expect(positionUpdateCount, greaterThanOrEqualTo(20));
        
        // Total time should be reasonable (under 2 seconds for 20 updates)
        expect(totalTime.inMilliseconds, lessThan(2000),
            reason: 'Rapid position updates should complete within 2 seconds');

        // Update intervals should be consistent
        if (updateIntervals.isNotEmpty) {
          final avgInterval = updateIntervals
              .map((d) => d.inMicroseconds)
              .reduce((a, b) => a + b) / updateIntervals.length;
          expect(avgInterval, lessThan(50000), // 50ms average interval
              reason: 'Position update intervals should be reasonable');
        }
      });
    });
    ...
    group('Large Scale Performance', () {
      testWidgets('Many port widgets render efficiently', (tester) async {
        ...
        expect(renderTime.inMilliseconds, lessThan(2000),
      ...
      testWidgets('Complex routing scene performs well', (tester) async {
            ...
            home: Scaffold(
              body: Stack(
                children: [
                  ...
                  Positioned(left: 50, top: 100, 
                    child: PhysicalInputNode(position: const Offset(50, 100))),
                  Positioned(left: 250, top: 80,
                    child: AlgorithmNodeWidget(
                      algorithmName: 'LFO 1', slotNumber: 1, position: const Offset(250, 80),
                      outputLabels: ['Triangle', 'Square'], outputPortIds: ['lfo1_tri', 'lfo1_sqr'])),
                  Positioned(left: 250, top: 180,
                    child: AlgorithmNodeWidget(
                      algorithmName: 'LFO 2', slotNumber: 2, position: const Offset(250, 180),
                      outputLabels: ['Sine', 'Sawtooth'], outputPortIds: ['lfo2_sin', 'lfo2_saw'])),
                  Positioned(left: 450, top: 100,
                    child: AlgorithmNodeWidget(
                      algorithmName: 'VCA', slotNumber: 3, position: const Offset(450, 100),
                      inputLabels: ['Audio', 'CV1', 'CV2'], outputLabels: ['Audio Out'],
                      inputPortIds: ['vca_audio', 'vca_cv1', 'vca_cv2'], outputPortIds: ['vca_out'])),
                  Positioned(left: 450, top: 220,
                    child: AlgorithmNodeWidget(
                      algorithmName: 'Filter', slotNumber: 4, position: const Offset(450, 220),
                      inputLabels: ['Audio', 'Cutoff'], outputLabels: ['Filtered'],
                      inputPortIds: ['filt_audio', 'filt_cutoff'], outputPortIds: ['filt_out'])),
                  Positioned(left: 650, top: 160,
                    child: AlgorithmNodeWidget(
                      algorithmName: 'Mixer', slotNumber: 5, position: const Offset(650, 160),
                      inputLabels: ['Input 1', 'Input 2', 'Input 3'], outputLabels: ['Mix Out'],
                      inputPortIds: ['mix_in1', 'mix_in2', 'mix_in3'], outputPortIds: ['mix_out'])),
                  Positioned(left: 850, top: 140,
        ...
        expect(performanceStopwatch.elapsedMilliseconds, lessThan(3000),
            reason: 'Complex routing scene should render within 3 seconds');
      });
    });

    group('Memory and Resource Management', () {
      testWidgets('Repeated widget rebuilds don\'t leak memory excessively', (tester) async {
          ...
          return MaterialApp(
            home: Scaffold(
              body: AlgorithmNodeWidget(
                ...
                position: Offset(100.0 + iteration * 10, 100.0 + iteration * 10),
        ...
        expect(memorySnapshots.length, lessThanOrEqualTo(20), // 2 ports × 10 iterations
    ...
    group('Frame Rate and Smoothness', () {
      testWidgets('Drag animations maintain smooth frame rate', (tester) async {
        ...
        final gesture = await tester.startGesture(tester.getCenter(find.byType(AlgorithmNodeWidget)));
        ...
        if (frameTimes.isNotEmpty) {
          ...
          expect(avgFrameTime, lessThan(33),
              reason: 'Frame rate should maintain at least 30fps (33ms per frame)');
        }
      });

      testWidgets('Multiple simultaneous animations perform smoothly', (tester) async {
        final dragCompletionTimes = <String, Duration>{};
              ...
              body: Stack(
                children: [
                  Positioned(
                    ...
                    top: 100,
                    child: AlgorithmNodeWidget(
                      algorithmName: 'Simultaneous 1',
                      slotNumber: 1,
                      position: const Offset(100, 100),
                      onDragEnd: () {
                        dragCompletionTimes['algo1'] = Duration(milliseconds: stopwatch.elapsedMilliseconds);
                      },
                    ),
                  ),
                  Positioned(
                    ...
                    child: PhysicalInputNode(
                      position: const Offset(300, 150),
                      onNodeDragEnd: () {
                        dragCompletionTimes['physical'] = Duration(milliseconds: stopwatch.elapsedMilliseconds);
                      },
                    ),
                  ),
                ],
        ...
        // Start multiple drags simultaneously (as much as possible in test environment)
        final algoCenter = tester.getCenter(find.byType(AlgorithmNodeWidget));
        final physicalCenter = tester.getCenter(find.byType(PhysicalInputNode));

        await tester.dragFrom(algoCenter, const Offset(50, 50));
        await tester.dragFrom(physicalCenter, const Offset(-30, 40));
        await tester.pumpAndSettle();

        // Both animations should complete in reasonable time
        expect(dragCompletionTimes.keys, contains('algo1'));
        expect(dragCompletionTimes.keys, contains('physical'));
        
        for (final completion in dragCompletionTimes.values) {
          expect(completion.inMilliseconds, lessThan(1000),
              reason: 'Simultaneous animations should complete within 1 second');
        }
      });
    });
  });
