  ...
  group('NodeLayoutAlgorithm', () {
    ...
    group('calculateLayout', () {
      ...
      test('positions physical inputs on left side', () {
        ...
        expect(result.physicalInputPositions['hw_in_1']!.y, lessThan(result.physicalInputPositions['hw_in_2']!.y));
      });
      ...
      test('positions physical outputs on right side', () {
        ...
        // Should be vertically centered with spacing
        expect(result.physicalOutputPositions['hw_out_1']!.y, lessThan(result.physicalOutputPositions['hw_out_2']!.y));
      });
      ...
      test('positions algorithms in center with slot ordering', () {
        final algorithms = [
          RoutingAlgorithm(
            ...
            algorithm: Algorithm(algorithmIndex: 1, guid: 'test1', name: 'Test Algorithm 1'),
            inputPorts: const [
              Port(
                id: 'algo_1_in',
                name: 'Input',
      ...
      test('handles complex routing scenarios', () {
        final physicalInputs = List.generate(3, (i) => Port(
          id: 'hw_in_${i + 1}',
          name: 'I${i + 1}',
          type: PortType.cv,
          direction: PortDirection.output,
        ...
        final algorithms = List.generate(5, (i) => RoutingAlgorithm(
          id: 'algo_$i',
          index: i,
          algorithm: Algorithm(algorithmIndex: i, guid: 'test$i', name: 'Test Algorithm $i'),
          inputPorts: [
            Port(
              id: 'algo_${i}_in',
              name: 'Input',
        ...
        for (final position in allPositions) {
          ...
          expect(position.x, lessThanOrEqualTo(NodeLayoutAlgorithm.canvasWidth));
          expect(position.y, greaterThanOrEqualTo(0.0));
          expect(position.y, lessThanOrEqualTo(NodeLayoutAlgorithm.canvasHeight));
        }
      });
    });
    ...
    group('detectConnectionOverlaps', () {
      test('detects no overlaps for non-crossing connections', () {
        ...
        final overlaps = layoutAlgorithm.detectConnectionOverlaps(connections, nodePositions);
    ...
    group('optimizeNodePositionsForConnections', () {
      test('adjusts positions to minimize connection overlap', () {
        ...
        final optimizedPositions = layoutAlgorithm.optimizeNodePositionsForConnections(
          algorithms,
          connections,
          initialPositions,
        );
    ...
    group('edge cases', () {
      ...
      test('handles no connections', () {
        final algorithms = List.generate(3, (i) => RoutingAlgorithm(
          id: 'algo_$i',
          index: i,
          algorithm: Algorithm(algorithmIndex: i, guid: 'test$i', name: 'Test Algorithm $i'),
          inputPorts: [
        ...
        final positions = algorithms.map((a) => result.algorithmPositions[a.id]!).toList();
      ...
      test('handles maximum node count performance', () {
        ...
        final algorithms = List.generate(20, (i) => RoutingAlgorithm(
          id: 'algo_$i',
          index: i,
          algorithm: Algorithm(algorithmIndex: i, guid: 'test$i', name: 'Test Algorithm $i'),
          inputPorts: [
