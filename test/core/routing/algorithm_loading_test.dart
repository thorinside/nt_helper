...
void main() {
  group('AlgorithmRouting Loading Tests', () {
    ...
    test('should load all algorithms from JSON without errors', () {
      ...
      for (final algorithmJson in algorithms) {
        ...
        try {
          ...
          final routing = AlgorithmRouting.fromSlot(
            slot,
            algorithmUuid: 'test_${guid}_${DateTime.now().millisecondsSinceEpoch}',
          );
      ...
      debugPrint('Successfully loaded: ${successfulAlgorithms.length} algorithms');
      ...
      expect(failedAlgorithms, isEmpty,
          reason: 'All algorithms should load without errors');
    });

    test('should ensure all factory algorithms have at least one input or output port (except special cases)', () {
      final algorithms = algorithmData['algorithms'] as List;
      ...
      for (final algorithmJson in algorithms) {
        ...
        try {
          ...
          if (!hasInputs && !hasOutputs) {
            algorithmsWithNoIO.add('$name ($guid) - inputs: ${routing.inputPorts.length}, outputs: ${routing.outputPorts.length}');
            debugPrint('WARNING: Algorithm $name ($guid) has no inputs or outputs!');
          } else {
            debugPrint('✓ $name ($guid) - inputs: ${routing.inputPorts.length}, outputs: ${routing.outputPorts.length}');
          }
        } catch (e) {
      ...
      expect(algorithmsWithNoIO, isEmpty,
          reason: 'All factory algorithms (except documented special cases) should have at least one input or output port');
    });
      ...
      for (final entry in testCases.entries) {
        ...
        if (algorithmJson != null) {
          try {
            ...
            debugPrint('✓ ${algorithmJson['name']} (${entry.key}) passed specific tests');
          } catch (e) {
      ...
      for (final algorithmJson in algorithms) {
        ...
        for (final modeName in modeParams.keys) {
          if (modeParamsWithNumbers.containsKey(modeName)) {
            expect(modeParams[modeName], equals(modeParamsWithNumbers[modeName]?.value),
                reason: 'Mode parameter $modeName should have matching values in both maps for $name ($guid)');
          }
        }
        
        // Notes algorithm should have no I/O parameters
        if (guid == 'note') {
          expect(ioParams, isEmpty,
...
Slot _createSlotFromJson(Map<String, dynamic> json) {
  ...
  for (int i = 0; i < parameters.length; i++) {
    ...
    final isEnumParam = paramInfo.unit == 1 || 
        (paramInfo.unit == 2 && paramInfo.max == 1); // Boolean is also enum-like
    
    if (isEnumParam) {
      // For mode parameters, create Add/Replace enums
      if (paramInfo.name.toLowerCase().endsWith('mode')) {
        parameterEnums.add(ParameterEnumStrings(
          algorithmIndex: 0,
          parameterNumber: paramInfo.parameterNumber,
          values: ['Add', 'Replace'],
        ));
...
int _getUnitType(Map<String, dynamic> param) {
  ...
  if (unitStr != null && unitStr.isNotEmpty) {
    ...
    if (unitStr == 'Hz' || unitStr == 'ms' || unitStr == 'mV' || 
        unitStr == '%' || unitStr == 'V' || unitStr == 'semitones' ||
        unitStr == 'frames' || unitStr == 'degrees') {
