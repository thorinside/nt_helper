...
void main() {
  group('Routing Implementations Direct Properties Tests', () {
    group('PolyAlgorithmRouting Direct Properties', () {
      test('should create ports with direct properties for gate-driven poly', () {
    ...
    group('MultiChannelAlgorithmRouting Direct Properties', () {
      test('should create ports with direct properties for stereo channels', () {
        ...
        final leftPorts = inputPorts.where((p) => 
          p.type == PortType.audio && p.stereoSide == 'left'
        ).toList();
        final rightPorts = inputPorts.where((p) => 
          p.type == PortType.audio && p.stereoSide == 'right'
        ).toList();
      ...
      test('should create ports with direct properties for declared inputs', () {
      ...
      test('should use direct properties in helper methods', () {
        ...
        final result = routing.validateConnection(multiChannelPort, masterMixPort);
        expect(result, isA<bool>()); // Just verify it returns a boolean, actual logic may allow the connection
      });
    });
    ...
    group('ConnectionDiscoveryService Direct Properties', () {
      test('should use direct properties for bus discovery', () {
        ...
        final connections = ConnectionDiscoveryService.discoverConnections([mockRouting]);
        ...
        final inputConnection = connections.firstWhere(
          (c) => c.connectionType == ConnectionType.hardwareInput && c.destinationPortId == 'input_1'
        );
        ...
        final outputConnection = connections.firstWhere(
          (c) => c.connectionType == ConnectionType.hardwareOutput && c.sourcePortId == 'output_1'
        );
        expect(outputConnection.sourcePortId, equals('output_1'));
        expect(outputConnection.busNumber, equals(13));
      });

      test('should discover algorithm-to-algorithm connections using direct properties', () {
        ...
        final connections = ConnectionDiscoveryService.discoverConnections([routing1, routing2]);

        // Should discover the algorithm-to-algorithm connection on bus 5
        final algoConnection = connections.firstWhere(
          (c) => c.sourcePortId == 'algo1_out' && c.destinationPortId == 'algo2_in',
          orElse: () => throw StateError('Algorithm-to-algorithm connection not found'),
        );

        expect(algoConnection.sourcePortId, equals('algo1_out'));
        expect(algoConnection.destinationPortId, equals('algo2_in'));
        expect(algoConnection.connectionType, equals(ConnectionType.algorithmToAlgorithm));
...
class _MockRouting extends AlgorithmRouting {
  ...
  @override
  List<Port> get inputPorts => _ports.where((p) => p.direction == PortDirection.input).toList();
  
  @override
  List<Port> get outputPorts => _ports.where((p) => p.direction == PortDirection.output).toList();
