  ...
  group('RoutingEditorCubit Layout Integration', () {
    ...
    group('applyLayoutAlgorithm', () {
      test('does nothing when routing editor not loaded', () async {
        ...
        verifyNever(() => mockLayoutAlgorithm.calculateLayout(
      ...
      test('applies layout algorithm when routing editor is loaded', () async {
        ...
        final algorithms = [
          RoutingAlgorithm(
            id: 'algo_0',
            index: 0,
            algorithm: Algorithm(algorithmIndex: 0, guid: 'test0', name: 'Test Algorithm 0'),
            inputPorts: const [
              Port(
                id: 'algo_0_in',
                name: 'Input',
        ...
        when(() => mockLayoutAlgorithm.calculateLayout(
          physicalInputs: any(named: 'physicalInputs'),
          physicalOutputs: any(named: 'physicalOutputs'),
          algorithms: any(named: 'algorithms'),
          connections: any(named: 'connections'),
        ...
        verify(() => mockLayoutAlgorithm.calculateLayout(
          physicalInputs: physicalInputs,
          physicalOutputs: physicalOutputs,
          algorithms: algorithms,
          connections: connections,
        ...
        expect(newState.nodePositions['hw_in_1'], equals(const NodePosition(x: 50.0, y: 100.0)));
        expect(newState.nodePositions['hw_out_1'], equals(const NodePosition(x: 750.0, y: 100.0)));
        expect(newState.nodePositions['algo_0'], equals(const NodePosition(x: 400.0, y: 100.0)));
      });
      ...
      test('applies layout and updates state correctly', () async {
        ...
        final layoutResult = LayoutResult(
          physicalInputPositions: {'hw_in_1': const NodePosition(x: 50.0, y: 100.0)},
          physicalOutputPositions: {},
          algorithmPositions: {},
          reducedOverlaps: [],
        ...
        expect(finalState.nodePositions['hw_in_1'], equals(const NodePosition(x: 50.0, y: 100.0)));
      });
    });
    ...
    group('layout algorithm service injection', () {
      test('allows injection of layout algorithm service', () {
        ...
        expect(() => newCubit.injectLayoutAlgorithm(newLayoutAlgorithm), returnsNormally);
    ...
    group('node positions state management', () {
      test('preserves existing node positions when not applying layout', () async {
      ...
      test('updates only algorithm positions when applying layout', () async {
        ...
        final initialPositions = {
          'algo_0': const NodePosition(x: 200.0, y: 150.0),
          'hw_in_1': const NodePosition(x: 100.0, y: 100.0),
          'custom_node': const NodePosition(x: 300.0, y: 200.0), // Non-algorithm node
        };
        ...
        final layoutResult = LayoutResult(
          physicalInputPositions: {'hw_in_1': const NodePosition(x: 50.0, y: 120.0)},
          physicalOutputPositions: {},
          algorithmPositions: {'algo_0': const NodePosition(x: 400.0, y: 180.0)},
          reducedOverlaps: [],
        ...
        expect(updatedState.nodePositions['algo_0'], equals(const NodePosition(x: 400.0, y: 180.0)));
        expect(updatedState.nodePositions['hw_in_1'], equals(const NodePosition(x: 50.0, y: 120.0)));
        
        // Custom node position should be preserved (not managed by layout algorithm)
        expect(updatedState.nodePositions['custom_node'], equals(const NodePosition(x: 300.0, y: 200.0)));
      });
    });
  });
}
