...
really being calibrated is the combination of the disting NT output and the VCO. Both may in fact be
...
review the video about that algorithm, which is here^53.
...
signal uses a power of 2 bits. Changing between bit depths is therefore discontinous.
...
6 None Type I
7 None Type II
53 https://www.youtube.com/watch?v=Tv5c8gPYwFs
...
Description
This algorithm generates chaotic CVs according to the Lorenz equations^54 or Rössler equations^55.
Each generates three CVs, named X, Y, and Z, which are available on separate outputs.
...
Name Min Max Default Unit Description
Attractor 0 1 0 Chooses which equations to use: Lorenz or Rössler.
Speed
range
...
Rho mod -10.00 10.0
0
0.00 V Modulates the Lorenz ‘r’ parameter.
Beta mod -10.00 10.0
0
0.00 V Modulates the Lorenz ‘b’ parameter.
54 https://en.wikipedia.org/wiki/Lorenz_system
55 https://en.wikipedia.org/wiki/Rössler_attractor
...
Tempo 30.0 240.
0
120.0 BPM The internal clock tempo.
...
Output to
breakout
0 1 0 If on, send MIDI clock to the breakout.
Output to
Select Bus
0 1 0 If on, send MIDI clock to the Select Bus.
Output to
USB
0 1 0 If on, send MIDI clock to USB.
...
Description
This algorithm performs real-time convolution^56 of the input signal and another signal (often called an
...
56 https://en.wikipedia.org/wiki/Convolution
57 https://www.youtube.com/watch?v=tDFZA9b2A0Y
...
There are plenty of free examples online, for example these^58 by EchoThief, or these^59 rather lovely
...
folder.
58 https://www.echothief.com/downloads/
59 https://web.archive.org/web/20190201211631/http://www.samplicity.com/bricasti-m7-impulse-responses/
...
Description
This algorithm implements a simple switch debouncer (see e.g. this Wikipedia page^60 ).
...
60 https://en.wikipedia.org/wiki/Switch#Contact_bounce
...
prime ratios. It was inspired by the theories of composer La Monte Young^61. An interesting read is the
pdf “Notes on The Theatre of Eternal Music” available in a number of places online e.g. here^62.
...
61 https://en.wikipedia.org/wiki/La_Monte_Young
62 https://www.melafoundation.org/theatre.pdf
...
63 https://en.wikipedia.org/wiki/Euclidean_rhythm
64 https://www.hisschemoller.com/blog/2011/euclidean-rhythms/
...
‘Trigger’.
Length 1 99 50 % Sets the output clock pulse width, if the ‘Output
type’ is ‘% of clock’.
...
depth video for that algorithm here^65.
...
65 https://www.youtube.com/watch?v=rn04rBEhIPM
66 https://www.youtube.com/watch?v=ZObDS3Hvfuo&t=90
...
It implements a granular synthesis^67 engine, taking as its source material either live audio input or
...
Connect an audio input, enable Record and enable Drone 1.
Connect CV/gate or MIDI, load a sample from the card, and play.
...
67 Essential reading on granular synthesis includes Microsound by Curtis Roads,
https://mitpress.mit.edu/9780262681544/microsound/
68 https://www.youtube.com/playlist?list=PLlY5j4QDwxWtIlNfDq_dCdryxR2hXTqvg
...
The filename is ‘saved .wav’ where ‘date and time’ come from the real-time clock.
...
first.
Simon Kirby gave a nice explanation of this (with video) in the Discord server, here^69.
...
and Trigger inputs, it also makes an interesting effect with more ad hoc triggers.
...
69 https://discord.com/channels/1268195809502036049/1292493724462485647/1330595944374468682
...
0.0 100.0 50.0 % Sets the pulse width of the square/pulse wave.
Offset -10.00 10.00 0.00 V Adds a constant voltage to the LFO output.
Asymmetr
y
-100.0 100.0 0.0 % Sets the asymmetry. This is somewhat like
...
parameter.
Clock with reset As ‘Clock’, but the clock also causes the LFO to reset its phase.
...
signals from arbitrary CVs (for an explanation of hysteresis see here^70 ).

GUI
The display shows the various logical operations being performed. Disabled channels are grayed out.

70 http://en.wikipedia.org/wiki/Hysteresis#Electronic_circuits
...
extended. You may like to review the video about the EX version, which is here^71.
...
record/play buttons on the module’s two encoders. This is available from our GitHub^72 if it’s not
...
71 https://www.youtube.com/watch?v=ZkL7v5CCzgc
72 https://github.com/expertsleepersltd/distingNT
...
A new folder with the name ‘saved ’ is created within the top level ‘looper’ folder
...
Some example scripts are included in the GitHub repository^73. You can download the .lua files and
...
document, which can be found on the firmware download page (here^75 ), right next to the download
link for this manual.

73 https://github.com/expertsleepersltd/distingNT/tree/main/lua
74 https://discord.com/channels/1268195809502036049/1331216813194739804
75 https://expert-sleepers.co.uk/distingNTfirmwareupdates.html
...
Description
This algorithm is an implementation of the open source Plaits^76 module by Émilie Gillet.
...
here^77. The documentation below assumes some familiarity with this.
...
Fine tune -100 100 0 cents Fine tuning control.
76 https://github.com/pichenettes/eurorack/tree/master/plaits
77 https://pichenettes.github.io/mutable-instruments-documentation/modules/plaits/manual/
...
1 28 13 The output bus for the send. Note that the send
always uses ‘Add’ output mode.
...
Trigger
voltage
-11.0 11.0 0.0 V Set the trigger voltage - the voltage the input signal
has to cross to trigger the oscilloscope.
Time
range
...
Description
This algorithm performs pitch shifting, using a time-domain algorithm^78.
...
78 By which we mainly mean that it’s not using a Phase Vocoder or similar technique.
...
It uses the six-operator FM engine from the open source Plaits^79 module by Émilie Gillet (exactly as
...
79 https://github.com/pichenettes/eurorack/tree/master/plaits
...
MPE
channels
Controls how the algorithm will respond to MPE.
See above.
I2C
channel
Sets the I2C channel.
Bend
range
...
Description
This algorithm is a polysynth where each voice is essentially a Rings (the open source Rings^80 module
by Émilie Gillet).
...
80 https://github.com/pichenettes/eurorack/tree/master/rings
...
apply equally well to this version. The YouTube playlist is here^81.
...
The input CV is sampled. If the quantizer has a gate input selected, the CV is sampled when
the gate goes high. Otherwise, it is sampled when it has changed sufficiently to select a new
note.
The input transpose is applied. This simply adjusts the CV in multiples of 12-TET semitones
i.e. one twelfth of a Volt.
The CV is quantized into the ‘lookup’ scale, yielding a note number.
The shift parameter is applied.
The note number selects a CV in the ‘output’ scale.
The output transpose is applied.
The output calibration is applied, if in use.
...
This works internally by removing the unwanted notes from the keyboard map, so if you’re already
using a .kbm file with unmapped notes you may get unexpected results.
...
‘MIDI (any octave)’ allows the quantizer to choose notes as held, but in any octave. For example, if
you hold any C and any G on your keyboard, the quantizer is free to choose any C or G.
‘MIDI (exact)’ allows the quantizer to choose only the actual notes played. For example, if you hold
C4 and G5, the quantizer may only choose C4 or G5, not any other C or G.
...
Description
This algorithm is an implementation of the open source Rings^82 module by Émilie Gillet.
...
manual for the original module is here^83. The documentation below assumes some familiarity with
this.
...
Fine tune -100 100 0 cents Provides a fine tuning control.
Structure 0 127 64 Controls the resonator 'structure'.
82 https://github.com/pichenettes/eurorack/tree/master/rings
83 https://pichenettes.github.io/mutable-instruments-documentation/modules/rings/manual/
...
Description
This algorithm implements the reverb and diffuser parts of the Clouds^84 module by Émilie Gillet.
...
Left input 1 28 1 The left audio input bus.
Right input 1 28 2 The right audio input bus.
84 https://github.com/pichenettes/eurorack/tree/master/clouds
...
simply as a “Turing Machine” after this module^85.
...
85 https://www.musicthing.co.uk/Turing-Machine/
...
spectrum of a sound (using an FFT) and uses that to resynthesize audio^86.
...
86 For the technically minded, it uses the phase vocoder technique.
87 https://www.youtube.com/watch?v=8HFm0zzLHRM
...
Description
This algorithm is a 16-step note sequencer, based on the sequencers in the Expert Sleepers FH-2^88
...
88 https://expert-sleepers.co.uk/fh2.html
...
It was designed particularly to support programs written for the Spin Semiconductor^89 FV-1 chip. This
popular IC has ended up in numerous products including some Eurorack modules (e.g. the FX Aid^90
and the Timiszoara^91 ). Its popularity and the relative ease of developing for it has led to a large
...
89 http://www.spinsemi.com
90 https://happynerding.com/category/fx-aid/
91 http://xaocdevices.com/main/timiszoara/

Where to get programs
Some example programs are included in the GitHub repository^92. You can just download the 3pot files
and copy them to your MicroSD card.
...
Otherwise, a quick internet search will turn up plenty of (unconverted) programs e.g. here^93.
...
92 https://github.com/expertsleepersltd/distingNT/tree/main/tools/three_pot
93 https://mstratman.github.io/fv1-programs/
94 https://github.com/HolyCityAudio/SpinCAD-Designer
...
video on that algorithm, here^95. The pitch shifter part of the EX algorithm was split out into a separate
...
95 https://www.youtube.com/watch?v=wSjuOA9yMBk
...
96 https://en.wikipedia.org/wiki/Oscillator_sync
...
video on that algorithm here^97. From the disting mk4 manual:
...
97 https://www.youtube.com/watch?v=htkdsLm0oa8
...
the separate Lua scripting document, which can be found on the firmware download page (here^99 ),
right next to the download link for this manual.

98 https://www.lua.org
99 https://expert-sleepers.co.uk/distingNTfirmwareupdates.html

MIDI SysEx reference.........................................................................................................................
The disting NT supports a variety of functions via MIDI System Exclusive (SysEx) messages.
The browser-based tools here^100 can be considered as informal documentation of these messages, as in
...
F0 00 21 27 6D 31 <16 bit index> F7
...
32H – Add algorithm
F0 00 21 27 6D 32 <16 bit specification value> <16 bit specification value> F7
Adds an algorithm to the current preset.
33H – Remove algorithm
F0 00 21 27 6D 33 F7
Removes an algorithm from the current preset.
34H – Load preset
F0 00 21 27 6D 34 F7
Loads or appends the preset specified by the filename string.
35H – New preset
F0 00 21 27 6D 35 F7
Starts a new preset (exactly as the “New Preset” menu).
36H – Save preset
F0 00 21 27 6D 36F7 Saves the preset. The option value can be 0 (prompt for file overwrite), 1 (never overwrite), or 2 (always overwrite). 37H – Move algorithm F0 00 21 27 6D 37 F7 Requests that an algorithm be moved to a new position in the list. 40H – Request algorithm guid F0 00 21 27 6D 40 F7 Queries the indexed algorithm in the current preset. The response uses the '40H – Algorithm guid’ format, below.

41H – Request preset name
F0 00 21 27 6D 41 F7
Requests the current preset name. Responds with '41H – Preset name', as below.
42H – Request number of parameters
F0 00 21 27 6D 42 F7
Requests the number of parameters in the indexed algorithm. Responds with '42H – Number of
parameters', as below.
43H – Request parameter info
F0 00 21 27 6D 43 <16 bit parameter number> F7
...
F0 00 21 27 6D 42 <16 bit number of parameters> F7
Contains the number of parameters in the indexed algorithm.
43H – Parameter info
F0 00 21 27 6D 43 <16 bit parameter number> <16 bit minimum>
<16 bit maximum> <16 bit default> F7
Contains information for the given parameter in the indexed algorithm.
44H – All parameter values
F0 00 21 27 6D 44 [<16 bit value>] F7
Contains the current values of all parameters in the indexed algorithm.
45H – Parameter value
F0 00 21 27 6D 45 <16 bit parameter number> <16 bit value> F7
...
49H – Enum strings
F0 00 21 27 6D 49 <16 bit parameter number>
[] F7
Contains the value strings for an 'enum' type parameter.

4BH – Mapping
F0 00 21 27 6D 4B <16 bit parameter number>
F7
