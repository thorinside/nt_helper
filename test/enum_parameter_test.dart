...
void main() {
  group('MCP Enum Parameter Support', () {
    ...
    test(
      'setParameterValue accepts enum string and converts to index',
      () async {
        ...
        when(
          mockController.getParametersForSlot(0),
        ).thenAnswer((_) async => [paramInfo]);
        when(
          mockController.getAlgorithmInSlot(0),
        ).thenAnswer((_) async => algorithm);
        when(
          mockController.getParameterEnumStrings(0, 0),
        ).thenAnswer((_) async => enumStrings);
        when(
          mockController.updateParameterValue(0, 0, 2),
        ).thenAnswer((_) async {});
    ...
    test(
      'setParameterValue still accepts numeric values for enum parameters',
      () async {
        // Mock parameter info for an enum parameter (unit = 1)
        final paramInfo = ParameterInfo(
          algorithmIndex: 0,
        ...
        when(
          mockController.updateParameterValue(0, 0, 1),
        ).thenAnswer((_) async {});
