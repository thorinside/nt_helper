...
3. In `lib/ui/widgets/packed_mapping_data_editor.dart`, locate the `onMidiEventFound` callback (starts line 683). After the existing note type check (lines 695-705 — the `if (type == MidiEventType.noteOn || type == MidiEventType.noteOff)` block that ends with its closing brace on line 705), add the 14-bit type handlers as an else-if chain before the final `_data = _data.copyWith(...)` call (line 707). The new code should be inserted between line 705 and line 707:
     ```dart
