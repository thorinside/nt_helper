...
void main() {
  test(
    'Capture Clock algorithm parameter values with flag analysis',
    () async {
      ...
      final distingDevice = devices
          .where((d) => d.name.contains('disting'))
          .firstOrNull;

      if (distingDevice == null) {
        fail(
          'No disting NT device found. Available devices: ${devices.map((d) => d.name).join(', ')}',
        );
      }
      ...
      try {
        ...
        if (capturedMessages.isEmpty) {
          ...
          for (
            var msgIndex = 0;
            msgIndex < capturedMessages.length;
            msgIndex++
          ) {
            final message = capturedMessages[msgIndex];
            print('\nMessage ${msgIndex + 1}:');
            print(
              'Hex dump: ${message.map((b) => b.toRadixString(16).padLeft(2, '0')).join(' ')}',
            );
